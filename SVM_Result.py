...
for kernel in ['linear', 'rbf', 'poly', 'sigmoid']:
        temp_classifier = SVC(kernel=kernel, random_state=32)
        temp_classifier.fit(x_train, y_train)
        temp_y_pred=temp_classifier.predict(x_test)
        score=accuracy_score(y_test,temp_y_pred)
        print("Accuracy score for Estimator {} is: {}%".format(kernel,round(score*100,2)))
        if score>best_accuracy:
            best_accuracy=score
            best_kernel = kernel

print("\nThe Best Accuracy is {} with Estimator value as {}".format(round(best_accuracy*100,2),best_kernel))
